...
#include "xj_robot_pnc/nav_goal.h"
#include <iostream>
#include <fstream>
#include <ctime>
#include <iomanip>
...
constexpr static const char* DEFAULT_DIR = "/home/wu/xj_robot/PATH";
...
private:
  ...
  void visualizePath(std::vector<geometry_msgs::PoseStamped> const& record_waypoint,ros::Publisher const& vis_pub);
  ...
  auto sendGoto(geometry_msgs::PoseStamped const& goal) -> bool;
  ...
  geometry_msgs::PoseStamped generateNewGoal(const geometry_msgs::PoseStamped& original_goal, 
                                              double step = 0.05, double max_radius = 1.0);

  void serviceThread();

  bool navServiceCB(xj_robot_pnc::nav_goal::Request& req, xj_robot_pnc::nav_goal::Response& res);
  void navigate_home();


private:
  StateValue cur_state_, last_state_;
  std::unique_ptr<tf2_ros::Buffer> tf_;
  std::unique_ptr<tf2_ros::TransformListener> tfl_;
  geometry_msgs::PoseStamped cur_goal_;
  bool ready_send_goto_ = false;
  size_t last_index_ = 0;
  bool keep_waypoint_ = false;
  bool is_going_home_ = false;
  
  std::thread service_thread;
  ros::ServiceServer nav_server;
  bool nav_result_ = false;
