...
{
  ros::NodeHandle nh;
  nh.param<int>("Joy_Liner_K", Joy_Liner_K, 25);
  nh.param<int>("Joy_Angular_K", Joy_Angular_K, 10);
  ...
  joy_ctrl_sub_ = nh.subscribe<sensor_msgs::Joy>("/joy",10,&CarMove::joy_ctrl_cb,this);
...
void CarMove::joy_ctrl_cb(const sensor_msgs::Joy::ConstPtr& joy)
{
  float left_rocker   = joy->axes[1];  // 0-axis[1]-->左摇杆上下移动-->控制轮毂电机前进后退
	...
	float A_button = joy->buttons[1];       // 3-buttons[0]-->A键-->循迹以及一级爬升到位
	float B_button = joy->buttons[3];       // 4-buttons[1]-->B键-->一级爬升到位至二级爬升到位
	float X_button = joy->buttons[4];       // 5-buttons[3]-->X键-->任意时刻退出至原点
...
{
	float wheel_base_ = 0.555; // 55.5cm
